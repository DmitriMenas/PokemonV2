...
class Player {
  ...
  // Check if the player hit a transition zone
  checkTransition() {
    this.inTransitionZone = false;
    let currentBox = null;
    console.log(transitionBoxes + "in function");
    for (let i = 0; i < transitionBoxes.length; i++) {
      ...
      if (
        ...
        this.inTransitionZone = true;
        currentBox = box;
        break;
      }
    }

    // Transition alert + level change
    if (this.inTransitionZone) {
      if (this.onLevel === currentBox) {
        console.log("Transition to previous level");
        // previousLevel();
      } else if (this.onLevel !== currentBox) {
        console.log("Transition to next level");
        alert("Working on it...");
        setTimeout(() => {
          // nextLevel();
          this.onLevel = currentBox;
...
class CollisionBox {
  ...
  draw() {
    // context.fillStyle = "rgba(250,0,0,0.40)";
...
//Transition Boxes (zones where level changed can be triggered)
let transitionBoxes = [];
transitionBoxes.push(new CollisionBox(15, 452, 20, 20));
...
function update() {
  ...
  transitionBoxes.forEach((box) => box.draw());
}
...
//     calibrating++;
//     console.log(
//       "Calibrated. Click the top left and bottom right corners of each box"
//     );
//   } else {
//     if (clickNum % 2 == 0) {
//       nextX = e.x - xoff;
//       nextY = e.y - yoff;
//     } else {
//       let collisionBoxString =
//         `barrierBoxes.push(new CollisionBox(` +
